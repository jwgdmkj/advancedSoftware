...
double** make_2d_Gaussian_Kernel(int w0, double sigma1) {
	...
	for (int x = -w0/2; x <= w0/2; x++) {
		for (int y = -w0/2; y <= w0/2; y++) {
			rr = sqrt(x * x + y * y);
			kernel[x + w0/2][y + w0/2] = (exp(-(rr * rr) / ss)) * sc;
			sum += kernel[x + w0/2][y + w0/2];
		}
	}
...
void my_cvSmooth(IplImage* src, IplImage* dst, int w0, double sigma1)
{
	...
	for (r = 0; r < m_height; r++) {
		for (c = 0; c < m_width; c++) { 
			...
			for (int i = 0; i < w0; i++) {
				for (int j = 0; j < w0; j++) {
					if ((r - w + i) < 0 || (r - w + i) >= m_height) {
						continue;
					}
					if (c - w + j < 0 || c - w + j >= m_width) {
						continue;
					}

					xx += cvGetReal2D(src, r - w + i, c - w + j) * kernel[i][j];
				}
			}

		//	mean = ceil(xx);
			cvSetReal2D(dst, r, c, xx);
			//////////////////////////////////////////////////////////////////////
